...
export function ModelsManager({
  ...
  return (
    <div className="space-y-8">
      ...
      {/* All models in one list, with their collections inline */}
      <section className="rounded-md border border-brand-border bg-brand-surface shadow-sm">
          ...
          {flatList.length === 0 ? (
            ...
            <ul className="space-y-1">
              {flatList.map((model) => (
                <li
                  key={model.id}
                  className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 rounded-md px-2 py-2 hover:bg-brand-light/60"
                >
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <p className="truncate text-sm font-medium text-brand-secondary">
                      {model.name.fr}
                    </p>
                    <p className="truncate text-xs text-brand-muted">{model.slug}</p>
